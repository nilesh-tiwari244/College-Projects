clc
clear all;
dim=140;                                                     %%%% width of x axis
snrlowerlimit=0;                                             %%%% x axis is ber vs snr
...
delta=[product1/cdf1 product2/cdf2];
...
for s=1:1:dim
    ...
    en=10^(k/10);
    snr(s,1)=k;
    for x=(hc(aa)/parts):hc(aa)/parts:hc(aa)-hc(aa)/parts
...
for s=1:1:dim
    ...
    for x=hc3/parts:hc3/parts:hc3-hc3/parts
        ...
        ff3=exp(((zz-(pi/6))^2)/(2*(pi/9)));
        ff=product(aaa)+(const(aaa))/(ff1*ff2*ff3);
        ggn=erfc(sqrt(en*x*0.5))*0.5*ff;
