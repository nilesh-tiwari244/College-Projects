...
fov = [pi*(60/180) pi*(40/180)];   % 20mm and 30mm lenses
                      ...
                      if(cuma==1)
                       ...
                       if (Sy2^2 + Sx2^2) > 1
                                summ(1,ttt)=0;
                                 ...
                                 [summ(1,ttt),h1,trialh,hcc]=func_10_calculating_equivalent_h(h1,h2,length(x),length(x),threshold,distribution);
                        end
                         if cuma < summ(1,ttt)   % for maximum expectation that h is greater than th
                                %if neth < trialh  % for maximum expected h value
                                pos=[qq1 jj1 qq2 jj2];
                                cuma=summ(1,ttt);
                                ttt=ttt+1;
                                h_max=h1;
...
% % hold on
% figure
% func_8_simulation_ber_snr_plot(neth,90,'-or');